        ...
        public string Generate()
        {
            using (WriteBlock($"namespace {@namespace}"))
            {
                using (WriteBlock($"internal class {@class}"))
                {
                    ...
                    foreach (var keyValue in symbols)
                        WriteLine($"public static IntPtr {keyValue.Value} {{ get; }}");
                    ...
                    using (WriteBlock($"static {@class}()"))
                    {
						...
						foreach (var keyValue in symbols)
							WriteLine($"{keyValue.Key} = CppSharp.SymbolResolver.ResolveSymbol(image, \"{keyValue.Key}\");");
					}
                }
            }
